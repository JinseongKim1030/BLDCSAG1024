...
# BLDCSAG1024 클래스 정의
class BLDCSAG1024:
    def __init__(self, prompt, negative_prompt, blending_start_percentage, device, batch_size=1, output_path = 'output.png'):
    ...
    def edit_image(
        ...
        for i, t in enumerate(timesteps):
            ...
            if sag_scale > 0.0:
                ...
                # SAG 마스킹
                degraded_latents, attn_mask = self.sag_masking(pred_x0, uncond_attn, map_size, t, eps)
...
class SamImageProcessor:
    ...
    def _load_image(self):
        # 이미지 로드 및 리사이즈
        pf_image = Image.open(self.img_path).convert("RGB").resize((1024, 1024), Image.LANCZOS)
        return pf_image
    ...
    def save_mask_image(self, max_score_mask):
        # 마스크 이미지를 저장
        mask_image = np.where(max_score_mask.numpy().reshape(1024, 1024, 1) != 0, [0, 0, 0, 255], [255, 255, 255, 0]).astype(np.uint8)
...
class ParametersTable:
    ...
    def wrap_text(self):
        ...
        wrapped_negative_prompt = "\n".join(textwrap.wrap(self.parameters["negative_prompt"], width=50))
        
        # Determine max height for the table
        max_height = max(wrapped_prompt.count('\n'), wrapped_negative_prompt.count('\n'))

        # Seed handling
        seed = 'no guidance' if self.parameters["sag_scale"] <= 0.0 else self.parameters["sag_scale"]
    ...
    def display_table(self):
        ...
        table = ax.table(cellText=table_data, colLabels=None, cellLoc='left', loc='center', colWidths=col_widths)
...
class ImageQualityEvaluator:
    def __init__(self, output_path, mask_path, original_path):
        ...
        # Load images
        self.original_image = Image.open(original_path).convert('RGBA').resize((1024, 1024), Image.BILINEAR)
        self.output_image = Image.open(output_path).convert('RGBA').resize((1024, 1024), Image.BILINEAR)
        self.mask_image = Image.open(mask_path).convert('RGBA').resize((1024, 1024), Image.BILINEAR)
    ...
    def image_to_tensor(self, image):
        ...
        tensor = torch.from_numpy(image).permute(2, 0, 1).unsqueeze(0)  # Shape: (1, C, H, W)
        return tensor
    ...
    def compute_ssim(self):
        ...
        ssim_value = ssim(np.array(self.original_rgb), np.array(self.generated_rgb), channel_axis=2)
        return ssim_value
    ...
    def compute_rgb_histogram_similarity(self):
        ...
        similarities = [cosine_similarity(h_orig, h_gen) for h_orig, h_gen in zip(histograms_original, histograms_generated)]
        ...
        weighted_average_similarity = sum(sim * weight for sim, weight in zip(similarities, weights)) / sum(weights)
    ...
    def show_metrics_table(self):
        ...
        table_data = [[name, f"{value:.4f}"] for name, value in zip(metrics_names, metrics_values)]
